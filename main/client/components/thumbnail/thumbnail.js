...
Template.thumbnail.onRendered(function() {
 ...
 var seriesDescription = this.find('.seriesDescription');
 seriesDescription.innerHTML = this.data.seriesDescription;

 //show instance
 var instance = this.data.instances[0];
 var element = this.find('.thumbnail');
...
function thumbnailDragHandler(e, target) {
    ...
    $('.imageViewer canvas').removeClass('faded');

    // Figure out what to do depending on what we're dragging over
    var viewportsDraggedOver = $(elemBelow).parents('.imageViewer');
    ...
    } else if (elemBelow.classList.contains('imageViewer') && elemBelow.classList.contains('empty')) {
...
function thumbnailDragEndHandler(e, target) {
    ...
    } else if (elemBelow.classList.contains('imageViewer') &&
...
Template.thumbnail.events({
  'touchstart .thumbnail, mousedown .thumbnail': function(e) {
