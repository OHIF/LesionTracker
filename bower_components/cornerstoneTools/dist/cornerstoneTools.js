    ...
    function mouseButtonTool(mouseToolInterface)
    {
        ...
        function mouseDownCallback(e, eventData) {
            ...
            if(cornerstoneTools.isMouseButtonEnabled(eventData.which, e.data.mouseButtonMask)) {
                ...
                // Now check to see if there is a text we can move
                // now check to see if we have a text that we can move
                //TODO: Aysel: Add check statement if mouse down location is on text callout. If so, call moveTextCallout function
                if(toolData !== undefined && mouseToolInterface.pointNearToolForText !== undefined) {
                    for(i=0; i < toolData.data.length; i++) {
                        data = toolData.data[i];
                        if(mouseToolInterface.pointNearToolForText(data, coords)) {
                            $(eventData.element).off('CornerstoneToolsMouseMove', mouseMoveCallback);
...
//TODO: Aysel: Create moveTextCallout function within cornerstoneTools module
